							...
							if(theta_l > 0 && theta_r > 0){
								for(Int_t p=0 ; p<Mult_right; p++){
									for(Int_t q=0 ; q<Mult_left; q++){
										...
										opa_foot_t = TMath::Sin(theta_l_fv)*TMath::Sin(theta_r_fv)*TMath::Cos(-phi_r_fv+phi_l_fv) + TMath::Cos(theta_l_fv)*TMath::Cos(theta_r_fv);
										...
										if(abs(dtheta_r)<4 && abs(dtheta_l)<4 &&  dist_min>temp_dist && TMath::Abs(xv)<30. && TMath::Abs(yv)<20.){
											dist_min = temp_dist;
											p_min = p;
											q_min = q;
											min_d_f_t=1;
